...
class RolloutStorage(object):
    def __init__(self, trajs_all, shuffle=True, norm_adv=False):
        ...
        if self.is_composite_state:
            ...
            self.obs_fovs = [
                torch.cat([traj['curr_states'][i]
                           for traj in trajs_all])
                for i in range(num_state_comp)
            ]
        else:
            self.obs_fovs = [
                torch.cat([traj['curr_states']
                           for traj in trajs_all])
            ]
...
class DCB_Stop_Data(Dataset):
    def __init__(self, root_dir, fix_labels, bbox_annos, pa, catIds):
        self.root_dir = root_dir
        self.pa = pa
        self.fix_labels = fix_labels
        self.catIds = catIds
    ...
    def __getitem__(self, idx):
        img_name, cat_name, condition, fixs, action, is_last, sid, spid = self.fix_labels[
            idx]
        imgId = cat_name + '_' + img_name
        if imgId in self.bboxes.keys():
            bbox = self.bboxes[imgId]
